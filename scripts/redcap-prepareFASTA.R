...
# ---- SETTINGS ----
metadata_path <- "add_filepath" # CSV with columns: sampleID, ngs_runid, redcap_repeat_instance
fasta_dir <- "add_filepath"           # Folder containing your FASTA files
output_dir <- "add_filepath"  # Output folder for renamed FASTAs
...
for (file_path in fasta_files) {
  ...
  sample_id <- str_extract(basename(file_path), "^[^_]+_[^_]+(?=_PER)")
  ...
  if (nrow(row) == 0) {
    ...
    message("⚠️ Multiple metadata entries for: ", sample_id, " — using the first")
    row <- slice(row, 1)
  }
  
  run_id   <- row$ngs_runid
